...
public class UserController {
    ...
    public List<UserDto.UserDetailResponse> getUsers() {
        return userRepository.findAll().stream()
                .map(user -> new UserDto.UserDetailResponse(
                        user,
                        farmMemberRepository.findByUserId(user.id()),
                        recordRepository.findByUserId(user.id()),
                        monthlyHistoryRepository.findByUserId(user.id()).orElse(null)
                ))
                .toList();
    }
    ...
    public UserDto.UserDetailResponse getUser(@PathVariable Long id) {
        ...
        return new UserDto.UserDetailResponse(
                user,
                farmMemberRepository.findByUserId(id),
                recordRepository.findByUserId(id),
                monthlyHistoryRepository.findByUserId(id).orElse(null)
        );
    }
    ...
    public UserDto.UserRecordsResponse getUserRecords(@PathVariable Long id) {
        UserDto.UserResponse user = findUser(id);
        return new UserDto.UserRecordsResponse(user, recordRepository.findByUserId(id));
    }
